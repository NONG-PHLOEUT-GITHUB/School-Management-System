  ...
  <v-container fluid class="px-0">
    <v-row>
      ...
      <v-col class="main-content elevation-2" cols="9">
        ...
        <v-card
          class="mx-auto pa-0"
          :title="selectedComment ? selectedComment.first_name : ''"
          subtitle="title"
          variant="flat"
        >
          <template v-slot:prepend>
          ...
          <template v-slot:append>
            <v-card-text
              >
              {{ formatTimestamp(selectedComment ? selectedComment.created_at : '') }}
              <v-btn variant="flat" icon="mdi-dots-vertical"></v-btn
            ></v-card-text>
          </template>
          <v-card-text
            >
            {{ selectedComment ? selectedComment.body : 'Select a comment to display' }}
            </v-card-text
          >
        </v-card>
      </v-col>
...
const formatTimestamp = (timestamp) => {
  ...
  if (secondsAgo < 60) {
    ...
    return `${formattedDate(date)}, ${formattedTime(date)} (${minutesAgo} minutes ago)`;
  } else if (hoursAgo < 24) {
    return `${formattedDate(date)}, ${formattedTime(date)} (${hoursAgo} hours ago)`;
  } else if (daysAgo < 2) {
    return `${formattedDate(date)}, ${formattedTime(date)} (1 day ago)`;
  } else {
    return `${formattedDate(date)}, ${formattedTime(date)} (${daysAgo} days ago)`;
  }
};
